/**
 * Restore grade 4 images from assets/grade4_<word>.png (all matching files).
 * Usage: node scripts/restore-grade4-images.mjs
 */
...
import { ASSETS_DIR, LESSON_TITLE, makeSquare } from "./lib/grade4-image-style.mjs";
...
function fileNameToWord(baseName) {
  return baseName.replace(/^grade4_/, "").replace(/_/g, " ");
}

loadEnvFile(join(rootDir, ".env"));
process.env.PROJECT_ROOT ??= rootDir;
assertServerNotRunning();
assertGrade123WriteAllowed("restore-grade4-images");
...
const files = readdirSync(ASSETS_DIR).filter((f) => /^grade4_.+\.png$/i.test(f));
let updated = 0;
let notFound = 0;
...
console.log(`Grade 4: ${stats.rows[0].with_image}/${stats.rows[0].total} with images`);
await client.close();
