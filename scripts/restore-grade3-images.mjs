/**
 * Restore grade 3 images from assets/grade3_<word>.png (all matching files).
 * Usage: node scripts/restore-grade3-images.mjs
 */
...
import { ASSETS_DIR, LESSON_TITLE, makeSquare } from "./lib/grade3-image-style.mjs";
...
function fileNameToWord(baseName) {
  return baseName.replace(/^grade3_/, "").replace(/_/g, " ");
}

loadEnvFile(join(rootDir, ".env"));
process.env.PROJECT_ROOT ??= rootDir;
assertServerNotRunning();
assertGrade123WriteAllowed("restore-grade3-images");
...
if (lessonRes.rows.length === 0) {
  console.error(`Lesson "${LESSON_TITLE}" not found. Run: node scripts/seed-grade3-lesson.mjs`);
  process.exit(1);
}
const lessonId = lessonRes.rows[0].id;

const files = readdirSync(ASSETS_DIR).filter((f) => /^grade3_.+\.png$/i.test(f));
let updated = 0;
let notFound = 0;
...
console.log(`Grade 3: ${stats.rows[0].with_image}/${stats.rows[0].total} with images`);
await client.close();
