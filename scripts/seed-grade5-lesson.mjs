/**
 * Creates lesson "grade 5" with vocabulary cards (idempotent).
 * Usage:
 *   node scripts/seed-grade5-lesson.mjs          — add missing cards only
 *   node scripts/seed-grade5-lesson.mjs --sync   — remove obsolete words, add/update to match seed-data
 */
...
import {
  ...
  GRADE5_LESSON_TITLE,
  GRADE5_TARGET_TOTAL,
} from "./seed-data/grade5-cards.mjs";
...
assertGrade123WriteAllowed("seed-grade5-lesson");
...
const lessonText =
  "Grade 5 English vocabulary — geometry, government, science, literature, and everyday words suited to Grade 5 learners (1500 cards). Cards without illustrations use simple single-word vocabulary.";

async function syncLessonWords(lessonId) {
  const target = new Map(
    GRADE5_CARDS.map((c) => [c.word.toLowerCase().trim(), c]),
  );
...
if (existing.rows.length > 0) {
  ...
  if (removed > 0) {
    console.log(`Removed ${removed} duplicate word(s) from "${GRADE5_LESSON_TITLE}".`);
  }
  if (sync) {
    const r = await syncLessonWords(lessonId);
    await client.close();
    console.log(
      `Synced "${GRADE5_LESSON_TITLE}" (id=${lessonId}): deleted ${r.deleted}, updated ${r.updated}, added ${r.added}. Total ${r.total} words.`,
    );
    process.exit(0);
  }
  ...
  if (n >= GRADE5_CARDS.length) {
    console.log(
      `Lesson "${GRADE5_LESSON_TITLE}" already exists (id=${lessonId}, ${n} words). OK.`,
    );
    await client.close();
    process.exit(0);
  }
  ...
  const missing = GRADE5_CARDS.filter((c) => !haveSet.has(c.word.toLowerCase()));
...
const insertedLesson = await client.query(
  `INSERT INTO lessons (title, text, word_count, level)
   VALUES ($1, $2, $3, $4)
   RETURNING id`,
  [GRADE5_LESSON_TITLE, lessonText, GRADE5_TARGET_TOTAL, "advanced"],
);
const lessonId = insertedLesson.rows[0].id;

const added = await insertCards(lessonId, GRADE5_CARDS);
await client.query(`UPDATE lessons SET word_count = $1 WHERE id = $2`, [added, lessonId]);

await client.close();
console.log(
  `Created lesson "${GRADE5_LESSON_TITLE}" (id=${lessonId}) with ${added} cards (target ${GRADE5_TARGET_TOTAL}).`,
);
