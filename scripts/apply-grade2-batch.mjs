/**
 * Apply grade2_<word>.png assets to grade 2 lesson (640×640 cream canvas).
 * Usage: node scripts/apply-grade2-batch.mjs batch1
 */
import { readFileSync, existsSync } from "node:fs";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";
import { createRequire } from "node:module";
import { BATCH_PROMPTS } from "./grade2-batch-prompts.mjs";
...
if (!batchArg || !BATCH_PROMPTS[batchArg]) {
  console.error("Usage: node scripts/apply-grade2-batch.mjs batch1|batch2|...");
  process.exit(1);
}
...
assertGrade123WriteAllowed("apply-grade2-batch");
...
await client.close();
console.log(
  `\n${batchArg}: ${ok} applied, ${miss} missing/failed. Lesson images total: ${count.rows[0].n}/${GRADE2_CARDS.length}`,
);
