/**
 * Apply grade4_<word>.png assets to grade 4 lesson (640×640 cream canvas).
 * Usage: node scripts/apply-grade4-batch.mjs batch1
 */
...
if (!batchArg || !BATCH_PROMPTS[batchArg]) {
  console.error("Usage: node scripts/apply-grade4-batch.mjs batch1|batch2|...|batch100");
  process.exit(1);
}
...
assertGrade123WriteAllowed("apply-grade4-batch");
...
await client.close();
console.log(
  `\n${batchArg}: ${ok} applied, ${miss} missing/failed. Lesson images total: ${count.rows[0].n}/${GRADE4_CARDS.length}`,
);
