/**
 * Creates lesson "grade 4" with vocabulary cards (idempotent).
 * Usage:
 *   node scripts/seed-grade4-lesson.mjs          — add missing cards only
 *   node scripts/seed-grade4-lesson.mjs --sync   — remove obsolete words, add/update to match seed-data
 */
...
import {
  ...
  GRADE4_LESSON_TITLE,
  GRADE4_TARGET_TOTAL,
} from "./seed-data/grade4-cards.mjs";
...
assertGrade123WriteAllowed("seed-grade4-lesson");
...
const lessonText =
  "Grade 4 English vocabulary — civics, advanced literacy, geometry, physical science, U.S. history, research skills, health, and the arts. Suited to Grade 4 learners (1000-card target).";

async function syncLessonWords(lessonId) {
  const target = new Map(
    GRADE4_CARDS.map((c) => [c.word.toLowerCase().trim(), c]),
  );
...
if (existing.rows.length > 0) {
  ...
  if (removed > 0) {
    console.log(`Removed ${removed} duplicate word(s) from "${GRADE4_LESSON_TITLE}".`);
  }
  if (sync) {
    const r = await syncLessonWords(lessonId);
    await client.close();
    console.log(
      `Synced "${GRADE4_LESSON_TITLE}" (id=${lessonId}): deleted ${r.deleted}, updated ${r.updated}, added ${r.added}. Total ${r.total} words.`,
    );
    process.exit(0);
  }
  ...
  if (n >= GRADE4_CARDS.length) {
    console.log(
      `Lesson "${GRADE4_LESSON_TITLE}" already exists (id=${lessonId}, ${n} words). OK.`,
    );
    await client.close();
    process.exit(0);
  }
  ...
  const missing = GRADE4_CARDS.filter((c) => !haveSet.has(c.word.toLowerCase()));
...
const insertedLesson = await client.query(
  `INSERT INTO lessons (title, text, word_count, level)
   VALUES ($1, $2, $3, $4)
   RETURNING id`,
  [GRADE4_LESSON_TITLE, lessonText, GRADE4_TARGET_TOTAL, "advanced"],
);
const lessonId = insertedLesson.rows[0].id;

const added = await insertCards(lessonId, GRADE4_CARDS);
await client.query(`UPDATE lessons SET word_count = $1 WHERE id = $2`, [added, lessonId]);

await client.close();
console.log(
  `Created lesson "${GRADE4_LESSON_TITLE}" (id=${lessonId}) with ${added} cards (target ${GRADE4_TARGET_TOTAL}).`,
);
