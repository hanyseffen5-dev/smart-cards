/**
 * Restore grade 1 images from assets/grade1_<word>.png (all matching files).
 * Usage: node scripts/restore-grade1-images.mjs
 */
...
import { ASSETS_DIR, LESSON_TITLE, makeSquare } from "./lib/grade1-image-style.mjs";
...
function fileNameToWord(baseName) {
  return baseName.replace(/^grade1_/, "").replace(/_/g, " ");
}

loadEnvFile(join(rootDir, ".env"));
process.env.PROJECT_ROOT ??= rootDir;
assertServerNotRunning();
assertGrade123WriteAllowed("restore-grade1-images");
...
const files = readdirSync(ASSETS_DIR).filter((f) => /^grade1_.+\.png$/i.test(f));
let updated = 0;
let notFound = 0;
...
console.log(`Grade 1: ${stats.rows[0].with_image}/${stats.rows[0].total} with images`);
await client.close();
