/**
 * Apply grade3_<word>.png assets to grade 3 lesson (640×640 cream canvas).
 * Usage: node scripts/apply-grade3-batch.mjs batch1
 */
...
import { assetPath, makeSquare, LESSON_TITLE } from "./lib/grade3-image-style.mjs";
...
if (!batchArg || !BATCH_PROMPTS[batchArg]) {
  console.error("Usage: node scripts/apply-grade3-batch.mjs batch1|batch2|...|batch50");
  process.exit(1);
}
...
assertGrade123WriteAllowed("apply-grade3-batch");
...
await client.close();
console.log(
  `\n${batchArg}: ${ok} applied, ${miss} missing/failed. Lesson images total: ${count.rows[0].n}/${GRADE3_CARDS.length}`,
);
