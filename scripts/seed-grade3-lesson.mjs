/**
 * Creates lesson "grade 3" with vocabulary cards (idempotent).
 * Usage:
 *   node scripts/seed-grade3-lesson.mjs          — add missing cards only
 *   node scripts/seed-grade3-lesson.mjs --sync   — remove obsolete words, add/update to match seed-data
 */
...
import {
  ...
  GRADE3_LESSON_TITLE,
  GRADE3_TARGET_TOTAL,
} from "./seed-data/grade3-cards.mjs";
...
assertGrade123WriteAllowed("seed-grade3-lesson");
...
const lessonText =
  "Grade 3 English vocabulary — geography, science, reading skills, health, environment, math, time, media, and citizenship. Suited to Grade 3 learners.";

async function syncLessonWords(lessonId) {
  const target = new Map(
    GRADE3_CARDS.map((c) => [c.word.toLowerCase().trim(), c]),
  );
...
if (existing.rows.length > 0) {
  ...
  if (removed > 0) {
    console.log(`Removed ${removed} duplicate word(s) from "${GRADE3_LESSON_TITLE}".`);
  }
  if (sync) {
    const r = await syncLessonWords(lessonId);
    await client.close();
    console.log(
      `Synced "${GRADE3_LESSON_TITLE}" (id=${lessonId}): deleted ${r.deleted}, updated ${r.updated}, added ${r.added}. Total ${r.total} words.`,
    );
    process.exit(0);
  }
  ...
  if (n >= GRADE3_CARDS.length) {
    console.log(
      `Lesson "${GRADE3_LESSON_TITLE}" already exists (id=${lessonId}, ${n} words). OK.`,
    );
    await client.close();
    process.exit(0);
  }
  ...
  const missing = GRADE3_CARDS.filter((c) => !haveSet.has(c.word.toLowerCase()));
...
const insertedLesson = await client.query(
  `INSERT INTO lessons (title, text, word_count, level)
   VALUES ($1, $2, $3, $4)
   RETURNING id`,
  [GRADE3_LESSON_TITLE, lessonText, GRADE3_TARGET_TOTAL, "intermediate"],
);
const lessonId = insertedLesson.rows[0].id;

const added = await insertCards(lessonId, GRADE3_CARDS);
await client.query(`UPDATE lessons SET word_count = $1 WHERE id = $2`, [added, lessonId]);

await client.close();
console.log(
  `Created lesson "${GRADE3_LESSON_TITLE}" (id=${lessonId}) with ${added} cards (target ${GRADE3_TARGET_TOTAL}).`,
);
